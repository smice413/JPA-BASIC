...
import jpabasic.reserve.domain.Review15;
...
public class MainNto1 {
    ...
    private static void saveReview(int grade, String comment) {
        ...
        try {
            ...
            Review15 review = new Review15(sight, grade, comment);
            em.persist(review);
            tx.commit();
        } catch (Exception e) {
            tx.rollback();
    ...
    private static void printReview() {
        ...
        try {
            ...
            Review15 review = em.find(Review15.class, 1L);
    ...
    private static void printReviewsBySightId() {
        ...
        try {
            tx.begin();
            TypedQuery<Review15> query = em.createQuery("select r from Review r where r.sight.id = :sightId order by r.id desc", Review15.class);
            query.setParameter("sightId", "S-01");
            List<Review15> results = query.getResultList();
            results.forEach(r -> {
    ...
    private static void printReviewsBySight() {
        ...
        try {
            tx.begin();
            Sight sight = em.find(Sight.class, "S-01");
            TypedQuery<Review15> query = em.createQuery("select r from Review r where r.sight = :sight order by r.id desc", Review15.class);
