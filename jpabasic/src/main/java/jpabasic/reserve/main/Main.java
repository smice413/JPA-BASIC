    ...
    public static void main(String[] args) throws IOException {
        ...
        try (BufferedReader reader = new BufferedReader(new InputStreamReader(System.in))) {
            while (true) {
                ...
                String line = reader.readLine(); // 콘솔에서 명령어 받음
                if (line == null) break;
                if (line.startsWith("new ")) { //새로운 user 생성(email새성)
                    handleNew(line);
                } else if (line.startsWith("get ")) { // user 존재여부 검색
                    handleGet(line);
                } else if (line.startsWith("change name ")) { //email 기준으로 이름 수정
                    handleChangeName(line);
                } else if (line.startsWith("remove ")) {//email로 데이터 삭제
                    handleRemove(line);
